 ...
 * 
 * Copyright © 2012-2014, FTB Launcher Contributors <https://github.com/Slowpoke101/FTBLaunch/> FTB
...
public class Library {
  ...
  public String url;
  public boolean localRepo;// when true the DL will be grabbed from the FTB Repo's and use the FTB
                           // hash check methods instead of the etag
  public List<String> checksums;// contains sha1 hashes of the file -- must check against all
                                // values!
  ...
  public String getPathNatives() {
    ...
    return _artifact.getPath(natives.get(OS.CURRENT).replace("${arch}", "64"));
    // return _artifact.getPath(natives.get(OS.CURRENT).replace("${arch}",
    // (Settings.getSettings().getCurrentJava().is64bits ? "64" : "32")));
  }

  public String getUrl() {
    return (url == null ? (Constants.MC_LIB) : url);
    // return (url == null ? (localRepo ? DownloadUtils.getCreeperhostLink(Locations.ftb_maven) :
    // Locations.mc_libs) : url);
  }
  ...
  private class Artifact {
    ...
    public String getPath(String classifier) {
      String ret =
          String.format("%s/%s/%s/%s-%s", domain.replace('.', '/'), name, version, name, version);
      if (classifier != null)
        ret += "-" + classifier;
      return ret + "." + ext;
    }
