...
public class CurseforgeUtil {
  ...
  public static List<Mod> getCommonFiles(ModPack oldPack, ModPack workPack) {
    ...
    /*
     * for(String hash : workPack.getFiles().keySet()){ Set<ModPackFile> hashMatching = (Set)
     * oldPack.getFilesByHash(hash).entrySet(); if(hashMatching.size() > 0){ for(ModPackFile file :
     * hashMatching){ if(file instanceof Mod){ common.add((Mod) file); } } } }
     */
