...
@SuppressWarnings("unused") public class DownloadUtils {
    ...
    public static long getTotalSize(List<DownloadInfo> downloads) {
        try {
            ...
            Collection<Long> size = new Parallel.ForEach<DownloadInfo, Long>(downloads)
                .apply(new Parallel.F<DownloadInfo, Long>() {
                        @Override public Long apply(DownloadInfo e) {
                            try {
                                if (e.size > 0) {
                                    return e.size;
                                } else {
