...
public class AuthlibDLer extends SwingWorker<Boolean, Void> {
  ...
  protected boolean addToClasspath(String location) {
    ...
    try {
      if (f.exists()) {
        ...
        this.getClass().forName("com.mojang.authlib.exceptions.AuthenticationException"); // will
                                                                                          // fail if
                                                                                          // not
                                                                                          // properly
                                                                                          // added
  ...
  protected boolean downloadJars() {
    try {
      jarURLs =
          new URL(Constants.MC_LIB + "com/mojang/authlib/" + authlibVersion + "/authlib-"
              + authlibVersion + ".jar");
    ...
    while (!downloadSuccess && (attempt < attempts)) {
      try {
        attempt++;
        Logger.logDebug("Connecting.. Try " + attempt + " of " + attempts + " for: "
            + jarURLs.toURI());
        ...
        outStream.close();
        if (dlConnection instanceof HttpURLConnection
            && (currentDLSize == fileSizes[0] || fileSizes[0] <= 0)) {
