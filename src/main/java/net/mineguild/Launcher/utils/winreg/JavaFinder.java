package net.mineguild.Launcher.utils.winreg;

/**
 * Java Finder by petrucio@stackoverflow(828681) is licensed under a Creative Commons Attribution
 * 3.0 Unported License. Needs WinRegistry.java. Get it at:
...
public class JavaFinder {
     ...
     *                  or WinRegistry.KEY_WOW64_32KEY to force access to 32-bit registry view, or
     *                  WinRegistry.KEY_WOW64_64KEY to force access to 64-bit registry view
    ...
    private static List<String> searchRegistry(String key, int wow64, final List<String> previous) {
        ...
        try {
            List<String> entries =
                WinRegistry.readStringSubKeys(WinRegistry.HKEY_LOCAL_MACHINE, key, wow64);
            for (int i = 0; entries != null && i < entries.size(); i++) {
                String val = WinRegistry
                    .readString(WinRegistry.HKEY_LOCAL_MACHINE, key + "\\" + entries.get(i),
                        "JavaHome", wow64);
     ...
     * Searches and returns results in this order: HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java
     * Runtime Environment (32-bits view) HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java Runtime
     * Environment (64-bits view) HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java Development Kit
     * (32-bits view) HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java Development Kit (64-bits
    ...
    protected static List<JavaInfo> findWinJavas() {
        ...
        javaExecs = JavaFinder
            .searchRegistry("SOFTWARE\\JavaSoft\\Java Development Kit", WinRegistry.KEY_WOW64_32KEY,
                javaExecs);
        javaExecs = JavaFinder
            .searchRegistry("SOFTWARE\\JavaSoft\\Java Development Kit", WinRegistry.KEY_WOW64_64KEY,
                javaExecs);
    ...
    protected static String getMacJavaPath(String javaVersion) {
        ...
        versionInfo =
            RuntimeStreamer.execute(new String[] {"/usr/libexec/java_home", "-v " + javaVersion});
    ...
    /**
     * @return: The path to a java.exe that has the same bitness as the OS (or null if no matching
     * java is found)
     * **************************************************************************
     */
