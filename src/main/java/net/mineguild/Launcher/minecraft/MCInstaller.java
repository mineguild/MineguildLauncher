...
public class MCInstaller {
    ...
    public static void setup(final ModPack pack, File launchPath, File gameDirectory,
        ...
        if (assets.size() > 0) {
            ...
            dlDialog = new MultithreadedDownloadDialog(MineguildLauncher.getParent(), assets,
        ...
        if (libraries.size() > 0) {
            long startTime = System.currentTimeMillis();
            dlDialog = new MultithreadedDownloadDialog(MineguildLauncher.getParent(), libraries,
        ...
        if (libraries.size() + assets.size() > 0) {
            JOptionPane.showMessageDialog(MineguildLauncher.getParent(),
                libraries.size() + assets.size() + " file(s) successfully downloaded!");
        }
        if (doLaunch) {
    ...
    private static List<DownloadInfo> getLibraries(ModPack pack) throws Exception {
        ...
        FileUtils
            .copyURLToFile(new URL(Constants.MG_FORGE + pack.getForgeVersion() + "/version.json"),
                forgeJson);
        ...
        URL url = new URL(
            Constants.MC_DL + "versions/{MC_VER}/{MC_VER}.json".replace("{MC_VER}", packbasejson));
        File json = new File(launchPath,
        ...
        local = new File(
            launchPath + "/versions/{MC_VER}/{MC_VER}.jar".replace("{MC_VER}", packbasejson));
        if (!local.exists()) {
            list.add(new DownloadInfo(new URL(Constants.MC_DL + "versions/{MC_VER}/{MC_VER}.jar"
                .replace("{MC_VER}", packbasejson)), local, local.getName()));
        }
        return list;
    }
    ...
    private static List<DownloadInfo> getAssets() throws Exception {
        ...
        FileUtils.copyURLToFile(new URL(
                "https://s3.amazonaws.com/Minecraft.Download/indexes/${version}.json"
                    .replace("${version}", version.getAssets())), json);
        ...
        Parallel.TaskHandler<DownloadInfo> th =
            new Parallel.ForEach<Entry<String, Asset>, DownloadInfo>(
                    ...
                    public DownloadInfo apply(Map.Entry<String, AssetIndex.Asset> e) {
                        try {
                            ...
                            if (!local.exists()) {
                                ...
                                return (new DownloadInfo(new URL(Constants.MC_RES + path), local,
                                    name, Lists.newArrayList(asset.hash), "sha1"));
                            }
                        } catch (Exception ex) {
    ...
    private static void launchMinecraft(ModPack pack, LoginResponse resp, JavaSettings set) {
        try {
            ...
            natDir.mkdirs();
            Version base = JsonFactory.loadVersion(new File(minecraftDir,
                "/versions/{MC_VER}/{MC_VER}.json".replace("{MC_VER}", packbasejson)));
            ...
            classpath.add(new File(minecraftDir,
                "/versions/{MC_VER}/{MC_VER}.jar".replace("{MC_VER}", packmcversion)));
            for (Library lib : base.getLibraries()) {
                classpath.add(new File(libDir, lib.getPath()));
            }

            Process minecraftProcess = MCLauncher
                .launchMinecraft(set.getJavaPath(), gameFolder, assetDir, natDir, classpath,
                    packjson.mainClass != null ? packjson.mainClass : base.mainClass,
                    packjson.minecraftArguments != null ?
                        packjson.minecraftArguments :
                        base.minecraftArguments,
                    packjson.assets != null ? packjson.assets : base.getAssets(),
                    pack.getMinecraftVersion(), resp.getAuth(), false, set);

            MineguildLauncher.MCRunning = true;
            MineguildLauncher.con.minecraftStarted();

            StreamLogger
                .prepare(minecraftProcess.getInputStream(), new LogEntry().level(LogLevel.UNKNOWN));
