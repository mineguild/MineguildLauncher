...
public class LoginWorker extends SwingWorker<LoginResponse, Void> {
    ...
    @Override protected LoginResponse doInBackground() throws Exception {
        try {
            auth.logIn();
            if (auth.isLoggedIn() && auth.canPlayOnline()) {
                ...
                LoginResponse response =
                    new LoginResponse(Integer.toString(auth.getAgent().getVersion()), "token",
                        auth.getSelectedProfile().getName(), null,
