...
@SuppressWarnings("serial") public class MCLaunchFrame extends JFrame {
    ...
    public MCLaunchFrame() {
        ...
        JLabel lblLaunchpath =
            DefaultComponentFactory.getInstance().createLabel("Install/Launch Path");
        ...
        memSlider.addMouseMotionListener(new MouseMotionAdapter() {
            @Override public void mouseDragged(MouseEvent e) {
                ...
                lblMemory.setText("Memory(" + FileUtils
                    .byteCountToDisplaySize(((long) source.getValue()) * 512 * 1024 * 1024) + ")");
            }
        });
        ...
        permGenComboBox
            .setModel(new DefaultComboBoxModel<String>(new String[] {"256m", "512m", "1024m"}));
    ...
    public void loadSettings() {
        ...
        if (javaSettings.getMaxMemory() >= 512) {
            memSlider.setValue(javaSettings.getMaxMemory() / 512);
        }
    ...
    public void launchMC() {
        Thread t = new Thread(new Runnable() {
            ...
            @Override public void run() {
                try {
                    MCInstaller.setup(ModpackBuilder.workPack, new File(launchPathField.getText()),
                        new File(getGameDirField().getText()),
                        ModpackBuilder.settings.getJavaSettings(), MineguildLauncher.res, true);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
