...
public class FileAddWorker<T> extends SwingWorker<Map<String, T>, Void> {
    ...
    @Override protected Map<String, T> doInBackground() throws Exception {
                ...
                .apply(new Parallel.F<File, Entry<T>>() {
                    ...
                    @Override public Entry<T> apply(File e) {
                        try {
                            ...
                            if (mods) {
                                if (entry.getKey().endsWith(".jar") || entry.getKey()
                                    .endsWith(".zip")) {
                                    List<ModInfo> modInfo = JsonFactory.loadModInfoFromJar(e);
                                    if (modInfo != null) {
                                        Entry<Mod> ret = new Entry<Mod>(entry.getKey(),
                                            Mod.fromModPackFile(entry.getValue()));
