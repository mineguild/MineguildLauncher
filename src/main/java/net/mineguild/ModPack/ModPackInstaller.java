...
public class ModPackInstaller {
    ...
    public static synchronized List<DownloadInfo> checkNeededFiles(final File installDirectory,
        ...
        List<DownloadInfo> result =
            (List<DownloadInfo>) new Parallel.ForEach<Entry<String, ModPackFile>, DownloadInfo>(
                pack.getFiles().entrySet()).withFixedThreads(OSUtils.getNumCores() * 2)
                .apply(new Parallel.F<Map.Entry<String, ModPackFile>, DownloadInfo>() {

                    @Override public DownloadInfo apply(Map.Entry<String, ModPackFile> entry) {
                        ...
                        if (packFile.sideMatches(side)) {
                            ...
                            try {
                                if (!localFile.exists()) {
                                    String dlPath =
                                        packFile.getHash().substring(0, 2) + "/" + packFile
                                            .getHash();
                                    DownloadInfo ret =
                                        new DownloadInfo(new URL(Constants.MG_MMP_FILES + dlPath),
                                            localFile, localFile.getName(),
                                            Lists.newArrayList(packFile.getHash()), "md5",
    ...
    public static synchronized void clearFolder(final File target, final ModPack pack,
        ...
        if (!target.isDirectory()) {
            throw new IOException(
                String.format("'%s' is no valid directory!", target.getAbsolutePath()));
        }
        boolean temp = false;
        try {
        ...
        Parallel.ForEach<File, Void> p = new Parallel.ForEach<File, Void>(FileUtils
            .listFiles(target, FileFilterUtils.trueFileFilter(), FileFilterUtils.trueFileFilter()));
        try {
            p.withFixedThreads(OSUtils.getNumCores() * 2).apply(new Parallel.F<File, Void>() {

                @Override public Void apply(File f) {
                    try {
                        ...
                        if (files.isEmpty()) {
                            if (doBackup) {
                                Logger.logInfo(String
                                    .format("Moving file %s to backup folder - not in pack!",
                                        f.getName()));
                                try {
                                    FileUtils.moveFileToDirectory(f, new File(backupDirectory,
                                            RelativePath
                                                .getRelativePath(target, new File(f.getParent()))),
                                        true);
                                } catch (FileExistsException e2) {
                                    Logger
                                        .logInfo(String.format("Not moving file %s!", f.getName()),
                                            e2);
                                    f.delete();
                                }
                            } else {
                                Logger.logInfo(
                                    String.format("Deleting file %s - not in pack!", f.getName()));
                                f.delete();
                            }
                        } else {
                            for (ModPackFile packFile : files.values()) {
                                if (packFile.sideMatches(side)) {
                                    Logger.logDebug(String
                                        .format("Leaving %s in there - side matches", f.getName()));
                                } else {
                                    if (doBackup) {
                                        Logger.logInfo(String.format(
                                            "Moving file %s to backup folder - side doesn't match!",
                                            f.getName()));
                                        try {
                                            FileUtils.moveFileToDirectory(f,
                                                new File(backupDirectory, f.getParent()), true);
                                        } catch (FileExistsException e2) {
                                            Logger.logInfo(
                                                String.format("Not moving file %s!", f.getName()),
                                                e2);
                                            f.delete();
                                        }
                                    } else {
                                        Logger.logInfo(String
                                            .format("Deleting file %s - side doesn't match!",
                                                f.getName()));
                                        f.delete();
                                    }
                                }
                        ...
                        Logger.logError(String.format("Unable to check hash of %s!", f.getName()),
                            e1);
                    }
    ...
    public static synchronized void clearFolder(final File target, final ModPack installedPack,
        ...
        Parallel.ForEach<Entry<String, ModPackFile>, Void> p =
            new Parallel.ForEach<Entry<String, ModPackFile>, Void>(
                installedPack.getFiles().entrySet());
        try {
            p.withFixedThreads(OSUtils.getNumCores() * 2)
                .apply(new Parallel.F<Entry<String, ModPackFile>, Void>() {

                    @Override public Void apply(Entry<String, ModPackFile> entry) {
                        String path = entry.getKey();
                                ...
                                if (doBackup) {
                                    try {
                                        FileUtils.moveFileToDirectory(localFile,
                                            new File(backupDirectory, RelativePath
                                                .getRelativePath(target,
                                                    new File(localFile.getParent()))), true);
                                    } catch (Exception e) {
                                        localFile.delete();
                                        Logger.logError(String
                                            .format("Deleting %s - error during backup", path), e);
                                    }
                                } else {
                                    localFile.delete();
