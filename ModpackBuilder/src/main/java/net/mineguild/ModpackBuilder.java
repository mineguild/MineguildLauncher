...
public class ModpackBuilder extends JFrame {
	...
	public ModpackBuilder() {
		...
		FormLayout layout = new FormLayout(
				"right:pref, 3dlu, pref, 3dlu, right:pref", // columns
				"p, 3dlu, p, 3dlu"); // rows
					...
					if (new File(file, "mods").exists()
						...
						if (versionField.getText().length() > 0) {
							...
							JOptionPane.showMessageDialog(
									ModpackBuilder.instance,
									"Version has to be non-empty!", "Error!",
									JOptionPane.ERROR_MESSAGE);
						}
					}
				} else {
					JOptionPane.showMessageDialog(ModpackBuilder.instance,
							"Invalid modpack-folder selected!", "Error!",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	...
	public static void main(String[] args) throws Exception {
		...
		new ModpackBuilder();
		/*
		 * System.exit(0); //getModpackDirectory(); Modpack newPack = new
		 * Modpack(modpackDirectory); File modpack_json = new
		 * File("newer_test.json"); Modpack oldPack =
		 * Modpack.fromJson(FileUtils.readFileToString(new
		 * File("new_test.json"))); newPack.addModpackFiles(); Gson g = new
		 ...
		 * placeUploadFiles(modpackDirectory.getAbsolutePath(),
		 * oldPack.getNew(newPack));
		 */
		// System.out.println(g.toJson(Modpack.getOld(oldPack, newPack)));
		/*
		 * Modpack m = new Modpack();
		 * m.setReleaseTime(System.currentTimeMillis()); List<File> list =
		 * (List<File>) FileUtils.listFiles(new File("testPack"),
		 * FileFilterUtils
		 * .notFileFilter(FileFilterUtils.suffixFileFilter(".dis")),
	...
	public static void placeUploadFiles(String basePath,
		...
		for (Map.Entry<String, String> entry : files.entrySet()) {
			...
			File newDirectory = new File(uploadDir, entry.getValue().substring(
					0, 2));
	...
	public static boolean getModpackDirectory(Component parent) {
		...
		FileNameExtensionFilter filter = new FileNameExtensionFilter(
				"Modpack_Json", "json", "mmp");
		fileChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
		// fileChooser.setFileFilter(filter);
		fileChooser
				.setDialogTitle("Select the directory of the modpack you want to update to.");
		int returnValue = fileChooser.showOpenDialog(parent);
		if (returnValue == JFileChooser.APPROVE_OPTION) {
			File selected = fileChooser.getSelectedFile();
			if ((new File(selected, "config")).exists()
					&& new File(selected, "config").isDirectory()
					&& (new File(selected, "mods")).exists()
					&& (new File(selected, "mods")).isDirectory()) {
